...
@Component({
  ...
  template: `
    <div class="rb-accordion border rounded-sm mb-2 overflow-hidden bg-card transition-fast">
      <div class="rb-accordion-header flex items-center justify-between px-4 py-3 cursor-pointer select-none"
        (click)="toggle()">
        <span class="text-sm font-semibold text-main">{{ title }}</span>
        <i class="fa-solid fa-chevron-down rb-accordion-icon transition-normal text-muted" [class.rotated]="expanded"></i>
      </div>
